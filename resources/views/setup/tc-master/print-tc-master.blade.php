      ...
      <div style="padding: 0px 7px 0px 7px; ">
         ...
         <?php
          ...
          <table style="width: 100%;margin-top:-0.5%;">
            <p style="padding-left: 86%; "> <span style="color: #000; font-size: 11px;">IS: {{$numericValue}} @if(isset($tc_master->part_no)) @if($tc_master->part_no=='Part I' || $tc_master->part_no=='Part II' || $tc_master->part_no=='Part III')({{$tc_master->part_no}}) @endif @endif: {{$year->year}}</span><br>
               <img src="{{asset('public')}}/printasset/isi.png" style="width: 30px;margin-top:6px;margin-left:15%;">  <img src="{{asset('public')}}/printasset/boxdesign1.jpg" style="width: 20px;margin-top:6px;margin-left:25%; "><br> <span style="color: #000; font-size: 11px;">CM/L:{{$tc_master->cml_no}}</span>
            </p>
         </table>
         <table  width="100%" style="width:96%;margin-right:-2%;margin-top: -1%; ">
            <tr>
               ...
               <td style="text-align:right ; color: #204f7d;font-size: 30px;font-weight: 700;"> <span style="">TEST CERTIFICATE</span><br> <b style="font-size: 11px; color: #000; font-weight: 700;">Format No F/08/17/R-03</b></td>
            </tr>
         </table>
         ...
         <table style="width:95%; margin: 25px 0px 0px 28px !important;border: 1px solid #899499;border-collapse: collapse; background-color:#6cbc4f;margin-top:3px; ">
            <tr >
               ...
               <th
                  ...
                  <span  style="width:64%;display:inline-block;text-transform:uppercase;line-height:normal;text-indent:15px;font-size:12px;font-family:'Font Awesome 5 Free';font-style:normal;color:#000;font-weight:600;border-bottom:1px dashed #000;text-transform:uppercase;margin-bottom: 0px; text-align: justify;"> {{$numericValue}} @if(isset($tc_master->part_no)) @if($tc_master->part_no=='Part I' || $tc_master->part_no=='Part II' || $tc_master->part_no=='Part III')({{$tc_master->part_no}}) @endif @endif: {{$year->year}}
                  </span>
               </th>
               <th
                  >
            ...
            <tr >
               ...
               <th
                  ...
                  <span  style="width:65%;display:inline-block;text-transform:uppercase;line-height:normal;text-indent:15px;font-size:12px;font-family:'Font Awesome 5 Free';font-style:normal;color:#000;font-weight:600;border-bottom:1px dashed #000;text-transform:uppercase;margin-bottom: 0px; text-align: justify;"> {{ $tc_master->date3 }}
                  </span>
               </th>
            </tr>
            <tr >
               ...
               <th
                  ...
                  <span  style="width:63%;display:inline-block;text-transform:uppercase;line-height:normal;text-indent:15px;font-size:12px;font-family:'Font Awesome 5 Free';font-style:normal;color:#000;font-weight:600;border-bottom:1px dashed #000;text-transform:uppercase;margin-bottom: 0px; text-align: justify;"> {{ $tc_master->date4 }}
                  </span>
               </th>
            </tr>
         </table>
         ...
         <table style="width: 96%;margin-top: %; margin-bottom: 10px;">
            <p style="font-size: 11px; margin-left: 5%;">We certify that the material described below fully conforms to <b>IS {{$numericValue}} @if(isset($tc_master->part_no)) @if($tc_master->part_no=='Part I' || $tc_master->part_no=='Part II' || $tc_master->part_no=='Part III')({{$tc_master->part_no}}) @endif @endif: {{$year->year}}</b>Chemical composition & physical properties of the product as tested in accordance with SCHEME OF INSPECTION AND<br> 
TESTING. Contained in the BIS certifications Marks L/C No. CM/L<b>{{ $tc_master->cml_no}}</b> are as indicated in your order (Please refer to <b>IS {{$numericValue}} @if(isset($tc_master->part_no)) @if($tc_master->part_no=='Part I' || $tc_master->part_no=='Part II' || $tc_master->part_no=='Part III')({{$tc_master->part_no}}) @endif @endif: {{$year->year}} </b>for details of specification requirements)</p>
         </table>
         <table cellspacing="0" border="1" width="100%" style="text-align:center;width:80%;margin: 0 auto;margin-top: px;background-image:url('{{asset('public')}}/printasset/bg.jpg');background-repeat: no-repeat;background-position: center center;">
            <!--Marksheet Table Heading Start-->
              ...
              <tr>
                ...
                <th style="font-size: 10px;border-bottom: none;">Lot No/Cast</th>
            ...
            <tr>
               ...
               <th style="font-size: 10px;width:30px;">Mass of Zinc<br>gm/m<sup>2</sup></th>
             ...
             <tr>
              ...
              <td style="font-size: 10px;"><b>(Min)<br>{{@$tc_master->yst_min}}</b></td>
            ...
            <?php
             if($tc_master->type=='MSGI' || @$tc_master->type=='CSGI')
            {
               $surfacecondition='GI';
            }
            elseif(@$tc_master->type=='MSBP' || @$tc_master->type=='CSBP')
            {
               $surfacecondition='BP';
            }
            else
        ...
        <tr>
            ...
            <td style="font-size: 11px; width: 205px;">ERW {{@$sizedesire->desier}} {{@$list->thikness}} {{@$list->grade}} {{ $surfacecondition }} {{@$list->ends}}</td>
            ...
            <td style="font-size: 11px;">{{ @$list->uniformitytest }}</td>
            <td style="font-size: 11px;">{{ is_numeric($list->massof_zn) ? number_format($list->massof_zn / 7.05, 2) : '-' }}</td>
...
threads conforming to IS 554 : 1999. Our Products<br> have been manufactured from IS marked HR Coil only.
            ...
            <tr>
               ...
               <td  style="font-size:9px;padding-left:32px;">@if(@$tc_master->is_no=='IS 4923')
                  <b>It is certified that each steel hollow section are supplied confirms to standard Dimensions and Mass tolerance & Straightness. Also, conforms to standard norms on Twist, Squareness of corner, Radii of Corner & Concavity/Convexity.</b><br> <span><b>* B/E- An angle of bevelled and root face conform to standard, P/E- Square cut ends, S/S- Pipe threads conform to IS 554 : 1999 & Fitted sockets conform to IS 1239 P-II : 2011, SWS- Pipe <br>
threads conforming to IS 554 : 1999. Our Products have been manufactured from IS marked HR Coil only<b></span><br><span>


                  @else
                  <b>It is certified that each steel tube is NDT/ Hydrostatically tested to test pressure of &nbsp;<font size="2">{{@$year->hp}}</font> MPa. Also Material supplied confirms to standard Dimensions
and Mass tolerances & Straightness. <br><span><b>* B/E- An angle of bevelled and root face conform to standard, P/E- Square cut ends, S/S- Pipe threads conform to IS 554 : 1999 & Fitted sockets conform to IS 1239 P-II : 2011, SWS- Pipe <br>
